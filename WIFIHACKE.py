...
class RunConfiguration:
    ...
    def __init__(self):
        ...
        # True means the program must find a valid handshake in order for this tool to recognize a handshake.
        ...
        self.IFACE_TO_TAKE_DOWN = ''  # Interface that this tool puts into monitor mode
    ...
    def ConfirmRunningAsRoot(self):
        if os.getuid() != 0:
            print R + ' [!]' + O + ' ERROR:' + G + ' this tool' + O + ' must be run as ' + R + 'root' + W
            exit(1)

    def ConfirmCorrectPlatform(self):
        if not os.uname()[0].startswith("Linux") and not 'Darwin' in os.uname()[0]:  # OSX support, 'cause why not?
            print O + ' [!]' + R + ' WARNING:' + G + ' this tool' + W + ' must be run on ' + O + 'linux' + W
            exit(1)

    def CreateTempFolder(self):
        from tempfile import mkdtemp

        self.temp = mkdtemp(prefix='this tool')
...
class RunEngine:
    ...
    def initial_check(self):
        ...
        if not program_exists('iwconfig'):
            print R + ' [!]' + O + ' this tool requires the program %s\n' % (R + 'iwconfig' + W)
            self.RUN_CONFIG.exit_gracefully(1)
    ...
    def scan(self, channel=0, iface='', tried_rtl8187_fix=False):
        ...
        """
        airodump_file_prefix = os.path.join(self.RUN_CONFIG.temp, 'this tool')
        ...
        try:
            ...
            while True:
                ...
                if not os.path.exists(csv_file) and time.time() - time_started > 1.0:
                    ...
                    print R + ' [!]' + O + ' this tool is unable to generate airodump-ng output files' + W
                    print R + ' [!]' + O + ' you may want to disconnect/reconnect your wifi device' + W
                    self.RUN_CONFIG.exit_gracefully(1)
...
def help():
    ...
    print head + '\n   EXAMPLE' + W
    print sw + '\t./this tool.py ' + W + '-wps -wep -c 6 -pps 600' + W
    print ''
